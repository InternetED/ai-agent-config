...
Ruling something out of scope is a scoping act, not a step on the route. When a ticket that already exists turns out to sit past the destination—mis-scoped while charting or exposed by a resolution—**close it** so it is unambiguously off the frontier. Leave one line in **Out of scope**: the gist plus why it is out of scope, linking the closed ticket. Keep it out of **Decisions so far**, which records the route actually walked; a scope boundary isn't a step on it.
